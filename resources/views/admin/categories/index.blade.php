                        ...
                        <table class="table table-striped table-hover table-bordered dataTable no-footer"
                            ...
                            @foreach($categories as $category)
                                <tr role="row" class="odd">
                                    ...
                                    <td>{{ blank_to_category($category->count) }}{{$category->name}}</td>
                                    <td class="center"><a href="{{$category->thumb}}">{{$category->url}}</a></td>
                                    <td class="center"><a href="">大声大声道</a></td>
                                    <td class="center"><input type="text" data-id="{{$category->id}}" class="sort_order"
                                                              value="{{$category->sort_order}}" style="width: 45px">
                                    </td>
                                    <td class="center" >

                                        <div class="fa-item col-md-3 col-sm-4">
                                            <i data-id="{{$category->id}}" @if($category->is_show==1) class="fa  is_show fa-check " value="1"
                                               @else class="fa is_show fa-times" value="0"
                                                    @endif></i> </div>
                                    </td>
                                    <td>
                                        <a href="category/{{$category->id}}/edit" class="btn btn-sm red"> edit
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        <a href="category/{{$category->id}}" class="btn btn-sm purple"
                                           data-method="delete"
